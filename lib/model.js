...
class Model {
  ...
  static destroy(options) {
    ...
    return Promise.try(() => {
      ...
      if (options.individualHooks) {
        // TODO: remove requestUser from here as this is an ugly hack to pass on this variable required for graphql endpoint security
        return this.findAll({ where: options.where, requestUser: options.requestUser, transaction: options.transaction, logging: options.logging, benchmark: options.benchmark })
  ...
  static restore(options) {
    ...
    return Promise.try(() => {
      ...
      if (options.individualHooks) {
        // TODO: remove requestUser from here as this is an ugly hack to pass on this variable required for graphql endpoint security
        return this.findAll({ where: options.where, requestUser: options.requestUser, transaction: options.transaction, logging: options.logging, benchmark: options.benchmark, paranoid: false })
      ...
      return this.constructor.QueryInterface[query](...args)
        ...
        .tap(() => {
          ...
          return Promise.map(this._options.include.filter(include => !(include.association instanceof BelongsTo ||
            ...
            if (includeOptions.concurrency) {
              return Promise.mapSeries(instances, instanceFunc);
            } else {
              return Promise.map(instances, instanceFunc);
            }
          });
        })
        .tap(result => {
