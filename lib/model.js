...
class Model {
  ...
  static update(values, options) {
    ...
    return Promise.try(() => {
      ...
      if (options.individualHooks) {
        // TODO: remove requestUser from here as this is an ugly hack to pass on this variable required for graphql endpoint security
        return this.findAll({where: options.where, requestUser: options.requestUser, transaction: options.transaction, logging: options.logging, benchmark: options.benchmark}).then(_instances => {
          instances = _instances;
          if (!instances.length) {
            return [];
          }
